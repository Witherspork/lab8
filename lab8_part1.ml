...
module MakeInterval (Endpoint : ORDERED_TYPE) =
  struct
    ...
    let contains (intvl : interval) (x : Endpoint.t) : bool =
      match intvl with
      | Empty -> false
      | Interval (a, b) -> Endpoint.compare x a >= 0 &&
...
let intvl2 = IntInterval.create 7 14 ;;
...
struct
  ...
  let create (a: endpoint) (b: endpoint) =
    if Endpoint.compare a b > 0 then Empty else Interval (a, b)
  ...
  let contains (intvl: interval) (x: Endpoint.t) : bool =
    match intvl with
    | Empty -> false
    | Interval (a, b) -> Endpoint.compare a x >= 0 &&
                         Endpoint.compare b x <= 0
